...
AS
	...
	SELECT @tOld_State_ID = STATE_ID FROM dbo.TPW_PJOB WHERE PJOB_ID = @inPJob_ID;
	IF @tOld_State_ID IS NULL
		RETURN -110;

	EXEC @tReturn = dbo.TPW_SERVICE_WF_EVENT_HANDLER @tOld_State_ID, @tEvent_ID OUTPUT, @inEvent_Name, @tNew_State_ID OUTPUT, @inCheck_State;
	IF @tReturn < 0
		RETURN @tReturn;
	...
	EXEC dbo.TPW_SERVICE_SET_SIGNAL @inPJob_ID, @tOld_State_ID, @inEvent_Name, @tNew_State_ID;
	...
	BEGIN
		...
		EXEC dbo.TPW_SERVICE_WF_LOG @inPJob_ID, @tOld_State_ID, @tEvent_ID, @tNew_State_ID, @inMessage;
	END;
